...
class Population(TinkerPopAble):
    '''
    ...
    def ofMap(pMap):
        '''
        create a Population from the given map
        '''
        pop=Population()
        pop.fromMap(pMap)
        return pop    
...
'''
    ...
    def __str__(self):
        text= ("%6s;%9s;%40s;%9s;%s;%s" % (self.isocode,self.wikiDataId,self.name,self.size,self.location,self.coords))    
        return text
